...
let internal = false;
let postgres = false;
let explicitBackend = false;
for (let index = 2; index < process.argv.length; index += 1) {
  const key = process.argv[index].replace(/^--/, "");
  if (key === "internal") { internal = true; continue; }
  if (key === "postgres") { postgres = true; continue; }
  if (!(key in options) || !process.argv[index + 1]) throw new Error("Usage: node scripts/setup-auth.mjs --output DIRECTORY --user USERNAME --origin ORIGIN [--postgres | --backend-url URL [--internal]]");
  if (key === "backend-url") explicitBackend = true;
  options[key] = process.argv[++index];
}
if (postgres && (internal || explicitBackend)) throw new Error("--postgres cannot be combined with --internal or --backend-url.");
...
if (!postgres && !internal && options.origin.startsWith("https:") && !options["backend-url"].startsWith("https:")) throw new Error("A production backend requires HTTPS; use --internal only for the combined container.");
...
const env = {
  ...
  ...(postgres ? { WORKSPACE_BACKEND: "postgres" } : {
